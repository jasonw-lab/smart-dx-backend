...
import com.smartdx.property.chatbot.config.ChatbotConfig;
...
/**
 * Google AI Studio (Gemini) LLMクライアント
 *
 * chatbot.llm.provider=gemini の場合に使用
 */
@Component
@ConditionalOnProperty(name = "chatbot.llm.provider", havingValue = "gemini")
@Slf4j
public class GeminiLlmClient implements LlmClient {
    ...
    public GeminiLlmClient(ChatbotConfig chatbotConfig, ObjectMapper objectMapper) {
        this.chatbotConfig = chatbotConfig;
        this.restTemplate = new RestTemplate();
        this.objectMapper = objectMapper;
        log.info("GeminiLlmClient initialized with model: {}", chatbotConfig.getLlm().getModel());
    }
    ...
    public LlmResponse complete(LlmRequest request) throws LlmException {
        ...
        try {
            String apiKey = chatbotConfig.getLlm().getApiKey();
            String model = chatbotConfig.getLlm().getModel();
            ...
            // デフォルトモデル
            if (model == null || model.isBlank() || model.equals("gpt-4o-mini")) {
